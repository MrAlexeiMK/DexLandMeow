...
public class EventListener {
    ...
    public void join(EntityJoinWorldEvent e) {
        if(Minecraft.getMinecraft().player != null) {
            if (e.getEntity() instanceof EntityPlayer) {
                if (Minecraft.getMinecraft().player.getGameProfile().getName().equals(((EntityPlayer) e.getEntity()).getGameProfile().getName())) {
    ...
    public void onChat(ClientChatReceivedEvent e) {
        ...
        String p_name = Minecraft.getMinecraft().player.getGameProfile().getName();
        if(msg.split(" ").length > 6 && msg.split(" ")[3].contains("был") && msg.split(" ")[4].contains("убит") && msg.split(" ")[5].contains("игроком")) {
